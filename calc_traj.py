...
class CalcTraj():
    ...
    def calcOrbslam(self, groundtruth, L):
        ...
        l_orb = np.zeros(self.frame)
        L_orb = 0
        for n in range(len(L) - 1):
            l_orb[n] = np.sqrt((x_orb[n + 1] - x_orb[n]) ** 2 + (y_orb[n + 1] - y_orb[n]) ** 2)
            L_orb = L_orb + l_orb[n]

        k_v=groundtruth[1]/L_orb
        x_vf = x_re * k_v
        y_vf = y_re * k_v
        z_vf = z_re * k_v
        return x_vf-x_vf[0], y_vf-y_vf[0], z_vf, r1_re, p1_re, ya1_re, t_orb
    ...
    def calcDroidslam(self, groundtruth, L):
        quotient = len(groundtruth[0]) // len(L)
        remainder = len(groundtruth[0]) % len(L)
        t = L[:, 1:4]
        q0 = L[:, 4]
        q1 = L[:, 5]
        q2 = L[:, 6]
        ...
        x_ = k_sfm*np.vstack([z_sfm[:], x_sfm[:], y_sfm[:]]).T  # colmap

        #_ = x_ - x_.mean(axis=0)  # genten
        y_ = y_ - y_.mean(axis=0)
